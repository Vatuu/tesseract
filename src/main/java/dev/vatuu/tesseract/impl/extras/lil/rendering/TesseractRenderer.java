...
import dev.vatuu.tesseract.impl.extras.lil.TesseractSettings;
...
import static dev.vatuu.tesseract.impl.extras.lil.rendering.DrawUtils.draw2Line;
...
public class TesseractRenderer {
    ...
    public void render(MatrixStack modelview, VertexConsumerProvider vertexConsumers, float projectionDistance){
        ...
        for(int i = 0; i < 4; i++) {
            draw2Line(projectedPoints, i, (i + 1) % 4, 8, consumer, model);
            draw2Line(projectedPoints, i + 4, ((i + 1) % 4) + 4, 8, consumer, model);
            draw2Line(projectedPoints, i, i + 4, 8, consumer, model);

            draw2Line(projectedPoints, i, i + 8, 4, consumer, model);
        }
        modelview.pop();
        updateRotation();
    }
